...
class SensorEspira(SensorBase):
    ...
    def generar_evento(self, nivel: float) -> dict:
        """
        Convierte el nivel de densidad en flujo vehicular (Q)
        usando la fórmula: Q = nivel * (1 - nivel) * Q_MAX [3, 4].
        """
        # Cálculo físico: el flujo es máximo cuando el nivel es 0.5 [3, 5]
        vehiculos = int(nivel * (1 - nivel) * self.Q_MAX)

        # Tiempos requeridos por el enunciado para el reporte de 30s [6]
        ahora = datetime.utcnow()
        inicio = ahora - timedelta(seconds=30)

        # Estructura exacta del JSON solicitada [6, 7]
        return {
